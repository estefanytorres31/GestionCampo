...
export const getAsistenciasController = async (req, res) => {
  try {
    const filters = {
      ...
      fecha_salida_inicio: req.query.fecha_salida_inicio || undefined,
      fecha_salida_fin: req.query.fecha_salida_fin || undefined,
    };

    const page = parseInt(req.query.page) || 1;
    const pageSize = parseInt(req.query.pageSize) || 10;

    const result = await AsistenciaService.getAsistencias(
      filters,
      page,
      pageSize
    );
...
export const obtenerAsistenciaPorId = async (req, res) => {
  try {
    const asistencia = await AsistenciaService.obtenerAsistenciaPorId(
      req.params.id
    );
    res.status(200).json({
...
export const actualizarAsistencia = async (req, res) => {
  ...
  try {
    const asistenciaActualizada = await AsistenciaService.actualizarAsistencia(
      parseInt(id_asistencia, 10),
      data
    );
    res
      .status(200)
...
export const eliminarAsistencia = async (req, res) => {
  ...
  try {
    const asistenciaEliminada = await AsistenciaService.eliminarAsistencia(
      parseInt(id_asistencia, 10)
    );
    res
      .status(200)
...
export const getLastAsistenciaByUsusario = async (req, res) => {
  ...
  try {
    const lastAsistencia = await AsistenciaService.obtenerUltimaAsistencia(
      id_usuario
    );
    res
      .status(200)
      .json({
        message: "Última asistencia obtenida exitosamente.",
        data: lastAsistencia,
      });
  } catch (error) {
    res.status(404).json({ message: error.message });
